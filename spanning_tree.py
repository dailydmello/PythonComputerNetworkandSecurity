...
def diameter (nodes, G):
    ...
    # number of vertices 
    number_of_nodes = len (G.keys())
    dist_mat=[]
    for i in range (0,number_of_nodes):  
        dist_mat.append([0,0,0,0,0,0,0]) #distance matrix

    for curr_node, node_neighbours in G.iteritems():#assign weights of 1 to nearby nodes #since list indexing starts at 0
        for neighbours in node_neighbours:
            dist_mat[curr_node-1][neighbours-1]=1

    for i in range(0,number_of_nodes): #setting nodes not adjacent to infinity
        for j in range (0,number_of_nodes):
            if (i!=j and dist_mat[i][j]!=1):
                dist_mat[i][j]=float('inf')

    
    # floyd warshall algorithm will store the distance from current node to every other node in a matrix

    for a in range (0,number_of_nodes):
        for b in range (0,number_of_nodes):
            for c in range (0,number_of_nodes):
                if (dist_mat[b][c]>dist_mat[b][a]+dist_mat[a][c]):
                    dist_mat[b][c] = dist_mat[b][a] + dist_mat[a][c]

    #loop through matrix and store largest distance as diameter

    maximum=0
    for i in range (0,number_of_nodes):
        for j in range (0,number_of_nodes):
            if maximum < dist_mat[i][j]:
                maximum = dist_mat[i][j]
...
def check_spanning_tree(nodes, G):
    ...
    current = 1
    while (len(visited)!=7):
        # loop to get all the node_neighbours to get into stack
        for curr_node, node_neighbours in G.iteritems():
            if (curr_node==current):
                for neighbours in node_neighbours:
                    if (check_visited(neighbours)==False):
                        stack.append(neighbours)
        #add current node to visited to keep track
        visited.append(current)
        
        # if stack is not empty pop top node on stack into current
        if (len(stack)!=0):
            current = stack.pop()

    visited.sort() #sort through stack to check for repeated cycles
